...
// NOLINTBEGIN
void glDebugCallback(GLenum, GLenum, GLuint id, GLenum severity, GLsizei,
...
int main(int argc, const char *const *argv) {
	...
	app_cli_context.add_option("--width", arg_map_width, "width of the cell map")
			->check(CLI::TypeValidator<std::size_t>());

	app_cli_context
			.add_option("--height", arg_map_height, "height of the cell map")
			->check(CLI::TypeValidator<std::size_t>());
	...
	app_cli_context
			.add_option("--step-delay", arg_counter,
	...
	ca_type_1dtotalistic_subcmd
			->add_option("-r,--range", cli_1dtotalistic_subcmd_data.range,
									 "describes range of the neighbourhood (0 ..<= 10)")
			->check(CLI::Range(CSIM::RuleConfig1DTotalistic::RANGE_LIM.x,
	...
	ca_type_1dtotalistic_subcmd->add_option(
			"-b,--birth-conditions", cli_1dtotalistic_subcmd_data.birth_conditions,
	...
	ca_type_1dbinary_subcmd
			->add_option("-r,--range", cli_1dbinary_subcmd_data.range,
	...
	std::transform(
			arg_colors.cbegin(), arg_colors.cend(), colors.begin(),
			[](std::uint32_t color) {
	...
	try {
		...
		if (ca_type_1dtotalistic_subcmd->parsed()) {
			rule_config = std::make_shared<CSIM::RuleConfig1DTotalistic>(
					...
					std::make_shared<CSIM::CShader>(
							"shaders/bin/1D_totalistic/comp.spv"));
		...
		std::shared_ptr<CSIM::Shader> render_shader =
				std::make_shared<CSIM::VFShader>(
		...
		glfwSetKeyCallback(
				window, [](GLFWwindow *win, int key, int, int action, int) {
					auto *renderer_ptr =
							static_cast<CSIM::Renderer *>(glfwGetWindowUserPointer(win));
					if (action == GLFW_PRESS) {
						...
						case GLFW_KEY_N:
							renderer_ptr->updateView({0.f, 0.f}, scale_value);
							break;
						case GLFW_KEY_M:
							renderer_ptr->updateView({0.f, 0.f}, 1.f / scale_value);
							break;
						// move keys
						case GLFW_KEY_W:
							renderer_ptr->updateView({0.f, offset_value}, 1.f);
							break;
						case GLFW_KEY_S:
							renderer_ptr->updateView({0.f, -offset_value}, 1.f);
							break;
						case GLFW_KEY_A:
							renderer_ptr->updateView({-offset_value, 0.f}, 1.f);
							break;
						case GLFW_KEY_D:
							renderer_ptr->updateView({offset_value, 0.f}, 1.f);
							break;
