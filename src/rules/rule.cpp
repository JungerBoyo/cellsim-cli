...
CSIM::Rule::Rule(std::shared_ptr<RuleConfig> rule_config)
	...
	glBindBufferBase(GL_UNIFORM_BUFFER, shconfig::BASE_CONFIG_UBO_BINDING_LOCATION, base_config_ubo_id_);
	glBindBufferBase(GL_UNIFORM_BUFFER, shconfig::CONFIG_UBO_BINDING_LOCATION, config_ubo_id_);

	// NOLINTBEGIN
	GLint active_resources{0};
	glGetProgramInterfaceiv(
			rule_config_->getShader()->program_id_,
			GL_UNIFORM_BLOCK,
			GL_ACTIVE_RESOURCES,
			&active_resources);
...
void CSIM::Rule1D::step(const CellMap& cell_map) noexcept {// NOLINT
	...
	config.map_resolution = {
		static_cast<std::int32_t>(cell_map.width_),
