...
CSIM::RuleConfig1DTotalistic::RuleConfig1DTotalistic( // NOLINT config is
		...
		: RuleConfig(std::move(step_shader)),
			config_serialized_(
					{{"Range", std::to_string(range)},
					 ...
					 {"Survival conditions",
						std::accumulate(survival_conditions.cbegin(), survival_conditions.cend(), std::string{},
														[](std::string str, std::size_t value) {
															return std::move(str) + std::to_string(value) + ',';
														})},
					 {"Birth conditions",
						std::accumulate(birth_conditions.cbegin(), birth_conditions.cend(), std::string{},
														[](std::string str, std::size_t value) {
															return std::move(str) + std::to_string(value) + ',';
														})}}) {
	config_.range = range;
	...
	// NOLINTBEGIN
	std::fill(config_.survival_conditions_hashmap, config_.survival_conditions_hashmap + MAX_OPTIONS,
						0);

	std::fill(config_.birth_conditions_hashmap, config_.birth_conditions_hashmap + MAX_OPTIONS, 0);
...
CSIM::RuleConfig2DCyclic::RuleConfig2DCyclic(std::int32_t range, std::int32_t threshold, bool moore,
	...
	std::memcpy(config_.offsets, offsets.data(),
							offsets.size() * sizeof(utils::Vec2<std::int32_t>)); // NOLINT interfacing with gl
}
CSIM::RuleConfig2DLife::RuleConfig2DLife(bool moore, bool state_insensitive, bool center_active,
		...
		: RuleConfig(std::move(step_shader)),
			config_serialized_(
					{{"State insensitive", state_insensitive ? "true" : "false"},
					 {"Center active", center_active ? "true" : "false"},
					 {"Kernel type", moore ? "Moore" : "Neumann"},
					 {"Survival conditions",
						std::accumulate(survival_conditions.cbegin(), survival_conditions.cend(), std::string{},
