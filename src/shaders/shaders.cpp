...
std::vector<char> CSIM::Shader::parseShader(const std::filesystem::path &path) {
	...
	if (!stream.good()) {
		throw std::runtime_error(
				fmt::format("failed to create stream from shader file {}", path.string()));
	}
...
CSIM::VFShader::VFShader(const std::filesystem::path &vsh_path,
	...
	glShaderBinary(1, &vsh_id_, GL_SHADER_BINARY_FORMAT_SPIR_V_ARB, vsh_binary.data(),
								 static_cast<GLsizei>(vsh_binary.size()));
	glShaderBinary(1, &fsh_id_, GL_SHADER_BINARY_FORMAT_SPIR_V_ARB, fsh_binary.data(),
								 static_cast<GLsizei>(fsh_binary.size()));
...
CSIM::CShader::CShader(const std::filesystem::path &csh_path)
	...
	glShaderBinary(1, &csh_id_, GL_SHADER_BINARY_FORMAT_SPIR_V_ARB, csh_binary.data(),
								 static_cast<GLsizei>(csh_binary.size()));
