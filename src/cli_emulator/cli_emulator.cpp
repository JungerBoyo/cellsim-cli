...
bool CSIM::CLIEmulator::draw(bool parse, bool backspace) {
	...
	if (just_parsed_counter_ = just_parsed_counter_ > 0 ? just_parsed_counter_ - 1 : 0;
		...
		const auto end_index =
				static_cast<std::int64_t>(cli_text_.find_first_of('\0', unsigned_next_insert_pos));

		/// if there is anything to parse
		if (end_index > blocking_cursor_pos_) {
			result = true;
			/// construct parse string taken from the cli buffer
			const std::string_view parse_str{std::next(cli_text_.cbegin(), next_insert_pos_),
			...
			if (parser_output.rdbuf()->in_avail() > 0) {
				...
				/// add new prompt
				const auto prompt_len_diff = static_cast<std::int64_t>(parser_output_str.length());
				...
				next_insert_pos_ +=
						static_cast<std::int64_t>(parse_str.length() + parser_output_str.length());
	...
	ImGui::InputTextMultiline("##source", cli_text_, cli_text_changed_, current_insert_pos_,
														clear_text_, static_cast<std::int32_t>(prompt_.length()) - 1,
														cursor_pos_, readonly);
	ImGui::End();
